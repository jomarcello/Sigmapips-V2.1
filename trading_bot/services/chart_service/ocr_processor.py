...
class ChartOCRProcessor:
    ...
    def __init__(self):
        ...
        self.vision_client = None # Initialize as None
        self.initialize_vision_client()
        # Fallback to Tesseract if Vision client fails or is not used
        # No fallback needed as per previous instruction, ensure Tesseract is setup if Vision is disabled
        logger.info(f"ChartOCRProcessor initialized.") # Removed Vision specific message
    ...
    def initialize_vision_client(self):
        # Commented out Google Vision initialization block
        # try:
        #     credentials_json = os.environ.get("GOOGLE_CREDENTIALS_JSON")
        #     credentials_path = os.environ.get("GOOGLE_APPLICATION_CREDENTIALS", "google_vision_credentials.json")
        #
        #     logger.info(f"Attempting to initialize Google Vision client.")
        ...
        #         # Check if the content is already a path or needs decoding
        #         try:
        #             # Attempt to load as JSON to see if it's JSON content
        #             json.loads(credentials_json)
        #             # If it's JSON content, write to default path if necessary
        #             if not os.path.exists(credentials_path):
        #                 with open(credentials_path, 'w') as f:
        #                     f.write(credentials_json)
        #                 logger.info(f"Credentials written to {credentials_path}")
        #             else:
        #                 logger.info(f"Credentials file {credentials_path} already exists.")
        #             credentials = service_account.Credentials.from_service_account_file(credentials_path)
        #         except json.JSONDecodeError:
        #             # Assume it's a path if not JSON
        #             logger.info("GOOGLE_CREDENTIALS_JSON appears to be a path.")
        #             credentials_path = credentials_json # Use this path directly
        #             if os.path.exists(credentials_path):
        #                credentials = service_account.Credentials.from_service_account_file(credentials_path)
        #             else:
        #                 logger.error(f"Credentials path specified in GOOGLE_CREDENTIALS_JSON does not exist: {credentials_path}")
        #
        #     elif os.path.exists(credentials_path):
        #         logger.info(f"Found Google credentials file at: {credentials_path}")
        #         credentials = service_account.Credentials.from_service_account_file(credentials_path)
        #     else:
        #         logger.warning("No GOOGLE_CREDENTIALS_JSON environment variable found and credentials file not found at default path.")
        #
        #     if credentials:
        #         # Removed scope specification as it's often handled by the client library or default service account permissions
        #         # scopes=['https://www.googleapis.com/auth/cloud-platform']
        #         self.vision_client = vision.ImageAnnotatorClient(credentials=credentials)
        #         logger.info("Google Vision client initialized successfully")
        #     else:
        #          logger.error(f"Google Vision credentials file not found or invalid: {credentials_path}")
        #          self.vision_client = None
        #
        # except Exception as e:
        #     logger.error(f"Failed to initialize Google Vision client: {str(e)}")
        #     self.vision_client = None
        pass # Keep the method structure but do nothing
    ...
    def process_image(self, image_path):
        ...
        try:
            ...
            # Always use Tesseract for now as Vision is disabled
            # if self.vision_client:
            #     # Create image object for Google Vision
            #     image = vision.Image(content=content)
            #
            ...
            #         logger.error(
            #             f'{text_response.error.message}\n'
            #             'For more info on error messages, check: '
            #             'https://cloud.google.com/apis/design/errors'
            #         )
            #         # Fallback to Tesseract on Vision API error
            #         logger.warning("Google Vision API error, falling back to Tesseract.")
            #         return self.process_with_tesseract(image_path, content)
            #
            #     texts = text_response.text_annotations
            #     if texts:
            #         # Extract the full detected text
            #         full_text = texts[0].description
            #         # logger.debug(f"Full text detected by Vision:\n{full_text}") # Log full text if needed
            #
            #         # Attempt to parse specific values from the full text or annotations
            #         parsed_data = self.parse_ocr_output(full_text, texts)
            #         # Get dominant colors if needed (commented out previously)
            #         # dominant_colors = self.get_dominant_colors(image)
            #         # parsed_data['dominant_colors'] = dominant_colors
            #         return parsed_data
            #     else:
            #         logger.warning("No text detected by Google Vision.")
            #         # Fallback to Tesseract if Vision detects no text
            #         return self.process_with_tesseract(image_path, content)
            # else:
            logger.info("Google Vision client not available or disabled, using Tesseract.")
            return self.process_with_tesseract(image_path, content)
            ...
            # Fallback to Tesseract on any unexpected error during Vision processing attempt
            try:
                with open(image_path, 'rb') as image_file:
                    content = image_file.read()
                return self.process_with_tesseract(image_path, content)
            except Exception as te:
                logger.error(f"Error during Tesseract fallback for {image_path}: {str(te)}")
                return None

    def process_with_tesseract(self, image_path, image_content=None):
        # Implementation of process_with_tesseract method
        pass

    # def get_dominant_colors(self, image):
    #     if not self.vision_client:
    #         logger.warning("Vision client not initialized, cannot get dominant colors.")
    #         return None
    #     try:
    ...
    #             })
    #         logger.info(f"Dominant colors detected: {len(colors_info)}")
    #         return colors_info
    #     except Exception as e:
